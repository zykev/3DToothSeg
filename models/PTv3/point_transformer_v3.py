...
from models.PTv3.serialization import encode
...
class SegHead(PointModule):
    def __init__(
        ...
        super().__init__()
        # self.in_channels = in_channels
        # self.nu = embed_channels

        self.cls = PointSequential(nn.Linear(in_channels, in_channels))

        self.edge_seg_head = PointSequential(nn.Linear(in_channels, in_channels))

        if norm_layer is not None:
            self.cls.add(norm_layer(in_channels), name="norm")
            self.edge_seg_head.add(norm_layer(in_channels), name="norm")
        if act_layer is not None:
            self.cls.add(act_layer(), name="act")
            self.edge_seg_head.add(act_layer(), name="act")
        
        self.cls.add(nn.Linear(in_channels, num_classes), name="cls_outproj")
        self.edge_seg_head.add(nn.Linear(in_channels, 2), name="edge_outproj")
    
    def forward(self, point: Point):
        point_seg_result = self.cls(point.feat)
        point_edge_seg_result = self.edge_seg_head(point.feat)

        return point.feat, point_seg_result, point_edge_seg_result
...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        self.embedding = Embedding(
            in_channels=in_channels + extra_pic_feat_channel,
        ...
        self.seghead = SegHead(
            in_channels=dec_channels[0] if not self.cls_mode else enc_channels[-1],
    ...
    def forward(self, data_dict):
        ...
        """
        batch_size = data_dict['feat'].shape[0]
        point_size = data_dict['feat'].shape[1]
        ...
        feat = torch.cat([feat[i] for i in range(batch_size)], dim=0).contiguous()
        ...
        if 'grid_coord' not in data_dict:
            ...
            coord = torch.cat(
                [data_dict['coord'][i] for i in range(batch_size)], dim=0
            ).contiguous()
            input_dict = {
                "feat": feat,
                "coord": coord,
            ...
            assert "grid_coord" in data_dict.keys(), "grid_coord is required for PointTransformerV3"
            grid_coord = torch.cat(
                [data_dict['grid_coord'][i] for i in range(batch_size)], dim=0
            ).contiguous()
        ...
        point_feat, seg_result, edge_seg_result = self.seghead(point)
        split0 = [o.item() for o in point.offset]
        ...
        return seg_result, edge_seg_result, point_feat
...
if __name__ == '__main__':
    ...
    input_feat = torch.rand((4, 16000, 6)).cuda()
    input_coords = torch.rand((4, 16000, 3)).cuda()
    input_grid = 0.05
    input_dict = {
        "feat": input_feat,
        "coord": input_coords,
    ...
    #     verbose=1,
    # )
    seg_result, edge_seg_result, point_feat = model(input_dict)
    pass